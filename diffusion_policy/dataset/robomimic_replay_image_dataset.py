...
class RobomimicReplayImageDataset(BaseImageDataset):
    def __init__(self,
        ...
        if hdf5_filter_key is not None:
            use_cache=False             #reload fresh because the filter key can be changed while the config stays intact.
...
def _convert_robomimic_to_replay(store, shape_meta, dataset_path, abs_action, rotation_transformer, 
    ...
    with h5py.File(dataset_path) as file:
        ...
        print(f'-----------feature-----------------')
        print(f'using filter key: {hdf5_filter_key}, total demos: {len(demo_names)}')
        print(f'----------------------------------')
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=n_workers) as executor:
                ...
                for key in rgb_keys:
                    ...
                    for episode_idx in range(len(demo_names)):
                        demo = demos[demo_names[episode_idx]]
